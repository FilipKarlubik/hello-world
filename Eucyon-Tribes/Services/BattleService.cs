...
namespace Eucyon_Tribes.Services
{
    public class BattleService : IBattleService
    {
        ...
        {
            List<Battle> battles = _db.Battles.Where(b => b.WinnerId == null && b.Fought_at <= DateTime.Now).ToList();
        ...
        public List<BattleResposeDto> GetBattles(int page, int itemCount, int id)
        {
            ...
            if (page < 1) page = 1;
            int totalCount = _db.Battles.Where(b => b.AttackerId == id || b.DefenderId == id).Count();
            if (totalCount < page * itemCount)
            {
                if (totalCount % itemCount == 0) page = totalCount / itemCount;
                else page = totalCount / itemCount + 1;
            }
            List<Battle> battlesInDB = _db.Battles.Where(b => b.AttackerId == id || b.DefenderId == id).OrderByDescending(u => u.Id).Skip((page - 1) * itemCount).Take(itemCount).ToList();
            List<BattleResposeDto> battles = new();
            foreach (Battle battle in battlesInDB)
            {
                if (battle.WinnerId == null)
                {
                    BattleResposeDto ba = new(battle.Id, battle.AttackerId,
