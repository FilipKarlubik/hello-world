...
namespace Eucyon_Tribes.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(IUserService userServices, ApplicationContext db
        ...
        {
            _message = _userServices.CreateUser(new UserCreateDto(name, password, email), kingdomName, worldId);
            return RedirectToAction("ListAllUsers");
        }
        ...
        public IActionResult AddSoldierToKingdomById(int kingdomId)
        {
            ...
            if (!_db.Armies.Any(a => a.KingdomId.Equals(kingdomId)))
            {
                Army army = new();
        ...
        private void ArmyUpdate(int armyId)
        {
            Army armyFromDb = _db.Armies.FirstOrDefault(a => a.Id.Equals(armyId));
            ...
            armyFromDb.Attack = soldiers.Sum(s => s.Attack);
            armyFromDb.Defense = soldiers.Sum(s => s.Defense);
            armyFromDb.HP = soldiers.Sum(s => s.HP);
            _db.Armies.Update(armyFromDb);
            _db.SaveChanges();
        }
