...
namespace Tribes.Tests
{
    public class PurchaseServiceTests
    {
        ...
        public void SeedDatabase()
        {
            ...
            var user = new User() { Email = "john@john.com", PasswordHash = "Johny123", Name = "John", ForgottenPasswordToken = "", VerificationToken = "" , Role = "Player"};
            _db.Users.Add(user);
        ...
        public void EnoughResourcesForCreatingSoldierTest()
        {
            ...
            Assert.True(_purchaseService.EnoughResourcesForCreatingSoldier(1));

            var gold = kingdom.Resources.FirstOrDefault(r => r.GetType().Name.Equals("Gold"));
            gold.Amount -= 200;
            _db.SaveChanges();

            Assert.False(_purchaseService.EnoughResourcesForCreatingSoldier(1));
        }
        ...
        public void EnoughResourcesForUpdatingSoldierTest()
        {
            ...
            Assert.True(_purchaseService.EnoughResourcesForUpgradingSoldier(new List<Soldier> { soldier },DateTime.Now));

            soldier.Level += 50;
            _db.SaveChanges();

            Assert.False(_purchaseService.EnoughResourcesForUpgradingSoldier(new List<Soldier> { soldier },DateTime.Now));
        }
