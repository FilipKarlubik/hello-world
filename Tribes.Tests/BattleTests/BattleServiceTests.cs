    ...
    public class BattleServiceTests : IDisposable
    {
        ...
        public void BattleService_Battle_DBChange()
        {
            ...
            ArmyFactory.Setup(i => i.CrateArmy(It.IsAny<List<Soldier>>(), kingdom2, 0))
                .Returns(new Army
                {
                    Soldiers = kingdom2.Resources.Where(r => r.GetType() == typeof(Soldier))
        ...
        [Fact]
        public void BattleService_CheckForBattles_Presist()
        {
            Battle battle = new Battle();
            battle.Fought_at = DateTime.Now.AddHours(1);
            Kingdom attacker = Context.Kingdoms.FirstOrDefault();
            User user2 = new User
            {
                Name = "user2",
            ...
            Assert.Equal(1, Context.Battles.Where(b => b.WinnerId == null).Count());
        }

        [Fact]
        public void BattleService_CheckForBattles_Battle()
        {
            Battle battle = new Battle();
            ...
            Assert.Equal(0, Context.Battles.Where(b => b.WinnerId == null).Count());
        }

        [Fact]
        public void BattleService_GetBattles_BattlesList()
        {
            Battle battle = new Battle();
            battle.Fought_at = DateTime.Now;
            Kingdom attacker = Context.Kingdoms.FirstOrDefault();
            ...
            List<BattleResposeDto> battleResponseDTOs= BattleService.GetBattles(1,1,1);

            Assert.Equal(1, battleResponseDTOs.Count);
            Assert.Equal(1, battleResponseDTOs[0].AttackerKingdomId);
            Assert.Equal(2, battleResponseDTOs[0].DefenderKingdomId);
            Assert.Equal("Battle not yet fought", battleResponseDTOs[0].Winner);
        }

        [Fact]
        public void BattleService_GetBattles_BattlesEmpty()
        {
            List<BattleResposeDto> battleResponseDTOs = BattleService.GetBattles(1, 1, 1);

            Assert.Equal(0, battleResponseDTOs.Count);
        }
    }
}
