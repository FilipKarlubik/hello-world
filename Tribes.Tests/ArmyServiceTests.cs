...
namespace Tribes.Tests
{
    ...
    public class ArmyServiceTests : IDisposable
    {
        ...
        public void ArmyServiceTest_GetAvailableUnits_AvailableUnitsDTO()
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                else
                {
                    Soldier soldier = new Soldier { Level = 2 };
                    kingdom.Resources.Add(soldier);
                    soldier.FinishedAt = DateTime.Now.AddMinutes(-5);
                    soldiers.Add(soldier);
                }
            }
        ...
        [Fact]
        public void ArmyServiceTest_GetAvailableUnits_DTO()
        {
            Kingdom kingdom = Context.Kingdoms.FirstOrDefault();
            List<Soldier> soldiers = new List<Soldier>();
            Army army1 = new Army();
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                else
                {
                    ...
                    if (i > 8)
                    {
                        soldier.FinishedAt = DateTime.Now.AddHours(1);
                    }
                }
            }
            Context.Armies.Add(army1);
            ...
            AvailableUnitsDTO output = ArmyService.GetAvailableUnits(kingdom.Id);

            Assert.Equal(0, output.NumberOfUnitsByLevel[0]);
            Assert.Equal(3, output.NumberOfUnitsByLevel[1]);
        }

        [Fact]
        public void ArmyServiceTest_GetAvailableUnits_Empty()
        {
            Kingdom kingdom = Context.Kingdoms.FirstOrDefault();
            List<Soldier> soldiers = new List<Soldier>();
            Army army1 = new Army();
            ...
            Assert.Equal(1, output.NumberOfUnitsByLevel.Count());
            Assert.Equal(0, output.NumberOfUnitsByLevel[0]);
        }

        [Fact]
        public void ArmyServiceTest_UnitsUnderConstruction_DTO()
        {
            Kingdom kingdom = Context.Kingdoms.FirstOrDefault();
            List<Soldier> soldiers = new List<Soldier>();
            ...
            for (int i = 0; i < 12; i++)
            {
                if (i < 6)
                {
                    ...
                    kingdom.Resources.Add(soldier);
                    soldiers.Add(soldier);
                    soldier.FinishedAt = dateTime.AddHours(2);
                }
                else
                {
                    Soldier soldier = new Soldier { Level = 2 };
                    ...
                    if (i > 8)
                    {
                        soldier.FinishedAt = dateTime.AddHours(1);
                    }
                }
            }
            Context.SaveChanges();

            UnitsUnderConstructionDTO[] output = ArmyService.UnitsUnderConstruction(kingdom.Id);

            Assert.Equal(2, output.Length);
            Assert.Equal(6, output[0].NumberOfSoldiersByLevel[0]);
            Assert.Equal(3, output[1].NumberOfSoldiersByLevel[1]);
            Assert.Equal(0, output[1].NumberOfSoldiersByLevel[0]);
        }

        [Fact]
        public void ArmyServiceTest_UnitsUnderConstruction_Empty()
        {
            Kingdom kingdom = Context.Kingdoms.FirstOrDefault();
            List<Soldier> soldiers = new List<Soldier>();
            for (int i = 0; i < 12; i++)
